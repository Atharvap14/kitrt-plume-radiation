# KiT-RT PlumeRadiation: Modular, HPC-Ready Radiative Transport Solver

[![License: Noncommercial](https://img.shields.io/badge/License-Noncommercial-yellow.svg)](LICENSE) [![GitHub Stars](https://img.shields.io/github/stars/KiT-RT/kitrt_code)](https://github.com/KiT-RT/kitrt_code/stargazers)
...
This repository adds **KiT-RT PlumeRadiation**, a CPU-first Discrete Transfer Method (DTM) extension for axisymmetric plume radiative heat-transfer studies. The v1 path is configured with `PROBLEM = PLUME_RADIATION`, `SOLVER = PLUME_DTM_SOLVER`, and `PLUME_BACKEND = CPU`; it reads structured `(z,r)` plume fields with `z_m,r_m,temperature_K,kappa_1_per_m`, can generate synthetic validation fields, and exports base heat-flux profiles as CSV plus optional VTK polydata.

Run the CPU plume example after building:

```bash
./build_omp/KiT-RT examples/plume_radiation/plume_cpu.cfg
```

Generate a standalone synthetic plume fixture:
...
For report-ready PNGs with visible legends and labels:
