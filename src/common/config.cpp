...
void Config::SetConfigOptions() {
    ...
    // Plume radiation related options
    AddEnumOption( "PLUME_BACKEND", _plumeBackend, PlumeBackend_Map, PLUME_BACKEND_CPU );
    AddStringOption( "PLUME_FIELD_FILE", _plumeFieldFile, string( "plume_field.csv" ) );
    AddBoolOption( "PLUME_SYNTHETIC_FIELD", _plumeSyntheticField, false );
    AddBoolOption( "PLUME_NOZZLE_SHADOW", _plumeNozzleShadow, false );
    AddBoolOption( "PLUME_WRITE_VTK", _plumeWriteVTK, false );
    AddDoubleOption( "PLUME_BASE_Z_M", _plumeBaseZ, 0.0 );
    AddDoubleOption( "PLUME_BASE_RADIUS_M", _plumeBaseRadius, 1.0 );
    AddUnsignedLongOption( "PLUME_BASE_SAMPLES", _plumeBaseSamples, 16 );
    AddUnsignedLongOption( "PLUME_RAY_POLAR_SAMPLES", _plumeRayPolarSamples, 12 );
    AddUnsignedLongOption( "PLUME_RAY_AZIMUTH_SAMPLES", _plumeRayAzimuthSamples, 24 );
    AddDoubleOption( "PLUME_RAY_STEP_M", _plumeRayStep, 0.01 );
    AddDoubleOption( "PLUME_RAY_MAX_DISTANCE_M", _plumeRayMaxDistance, 5.0 );
    AddDoubleOption( "PLUME_INCOMING_INTENSITY", _plumeIncomingIntensity, 0.0 );
    AddDoubleOption( "PLUME_NOZZLE_RADIUS_M", _plumeNozzleRadius, 0.0 );
    AddDoubleOption( "PLUME_NOZZLE_LENGTH_M", _plumeNozzleLength, 0.0 );
    AddDoubleOption( "PLUME_SYNTHETIC_TEMPERATURE_K", _plumeSyntheticTemperature, 1200.0 );
    AddDoubleOption( "PLUME_SYNTHETIC_KAPPA_1_PER_M", _plumeSyntheticKappa, 0.1 );
    AddDoubleOption( "PLUME_SYNTHETIC_Z_MAX_M", _plumeSyntheticZMax, 5.0 );
    AddDoubleOption( "PLUME_SYNTHETIC_R_MAX_M", _plumeSyntheticRMax, 5.0 );
    AddUnsignedLongOption( "PLUME_SYNTHETIC_Z_CELLS", _plumeSyntheticZCells, 32 );
    AddUnsignedLongOption( "PLUME_SYNTHETIC_R_CELLS", _plumeSyntheticRCells, 32 );
...
void Config::SetPostprocessing() {
    ...
    _plumeFieldFile    = std::filesystem::path( _inputDir ).append( _plumeFieldFile ).lexically_normal();
    ...
    // --- Solver setup ---
    {
        ...
        if( GetSolverName() == PLUME_DTM_SOLVER && GetProblemName() != PROBLEM_PlumeRadiation ) {
            ErrorMessages::Error( "PLUME_DTM_SOLVER requires PROBLEM=PLUME_RADIATION.", CURRENT_FUNCTION );
        }
        if( GetProblemName() == PROBLEM_PlumeRadiation && GetSolverName() != PLUME_DTM_SOLVER ) {
            ErrorMessages::Error( "PROBLEM=PLUME_RADIATION requires SOLVER=PLUME_DTM_SOLVER.", CURRENT_FUNCTION );
        }
        ...
        for( unsigned short idx_volOutput = 0; idx_volOutput < _nVolumeOutput; idx_volOutput++ ) {
            switch( _solverName ) {
                ...
                case CSD_MN_SOLVER:
                    ...
                    break;
                case PLUME_DTM_SOLVER:
                    supportedGroups = { MINIMAL };
                    if( supportedGroups.end() == std::find( supportedGroups.begin(), supportedGroups.end(), _volumeOutput[idx_volOutput] ) ) {
                        ErrorMessages::Error( "PLUME_DTM_SOLVER only supports volume output MINIMAL.\nPlease check your .cfg file.",
                                              CURRENT_FUNCTION );
                    }
                    break;
                default:
    ...
    // Plume radiation postprocessing
    if( _problemName == PROBLEM_PlumeRadiation ) {
        if( _plumeBaseRadius < 0.0 ) ErrorMessages::Error( "PLUME_BASE_RADIUS_M must be non-negative.", CURRENT_FUNCTION );
        if( _plumeBaseSamples == 0 ) ErrorMessages::Error( "PLUME_BASE_SAMPLES must be positive.", CURRENT_FUNCTION );
        if( _plumeRayPolarSamples == 0 ) ErrorMessages::Error( "PLUME_RAY_POLAR_SAMPLES must be positive.", CURRENT_FUNCTION );
        if( _plumeRayAzimuthSamples == 0 ) ErrorMessages::Error( "PLUME_RAY_AZIMUTH_SAMPLES must be positive.", CURRENT_FUNCTION );
        if( _plumeRayStep <= 0.0 ) ErrorMessages::Error( "PLUME_RAY_STEP_M must be positive.", CURRENT_FUNCTION );
        if( _plumeRayMaxDistance <= 0.0 ) ErrorMessages::Error( "PLUME_RAY_MAX_DISTANCE_M must be positive.", CURRENT_FUNCTION );
        if( _plumeNozzleRadius < 0.0 ) ErrorMessages::Error( "PLUME_NOZZLE_RADIUS_M must be non-negative.", CURRENT_FUNCTION );
        if( _plumeNozzleLength < 0.0 ) ErrorMessages::Error( "PLUME_NOZZLE_LENGTH_M must be non-negative.", CURRENT_FUNCTION );
        if( _plumeSyntheticTemperature < 0.0 ) ErrorMessages::Error( "PLUME_SYNTHETIC_TEMPERATURE_K must be non-negative.", CURRENT_FUNCTION );
        if( _plumeSyntheticKappa < 0.0 ) ErrorMessages::Error( "PLUME_SYNTHETIC_KAPPA_1_PER_M must be non-negative.", CURRENT_FUNCTION );
        if( _plumeSyntheticZMax <= 0.0 ) ErrorMessages::Error( "PLUME_SYNTHETIC_Z_MAX_M must be positive.", CURRENT_FUNCTION );
        if( _plumeSyntheticRMax <= 0.0 ) ErrorMessages::Error( "PLUME_SYNTHETIC_R_MAX_M must be positive.", CURRENT_FUNCTION );
        if( _plumeSyntheticZCells < 2 ) ErrorMessages::Error( "PLUME_SYNTHETIC_Z_CELLS must be at least 2.", CURRENT_FUNCTION );
        if( _plumeSyntheticRCells < 2 ) ErrorMessages::Error( "PLUME_SYNTHETIC_R_CELLS must be at least 2.", CURRENT_FUNCTION );
    }
